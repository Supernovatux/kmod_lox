...
{
	return NUMBER_VAL((long)ktime_get_ns() / CLOCKS_PER_SEC);
}
...
static void runtimeError(const char *format, ...)
{
	...
	vprintk(format, args);
	va_end(args);
	printBuff(lox_output_buffer, "\n");

	CallFrame *frame = &vm.frames[vm.frameCount - 1];
	size_t instruction =
		frame->ip - frame->closure->function->chunk.code - 1;
	int line = frame->closure->function->chunk.lines[instruction];
	printBuff(lox_output_buffer, "[line %d] in script\n", line);
...
{
	// FUNBEGIN;
	// printBuff(lox_output_buffer, "vm.stackTop : %p", vm.stackTop);
	*vm.stackTop = value;
	vm.stackTop++;
	// FUNEND;
}
